import { motion } from "framer-motion";
import fb from "../assets/fb.png"
import zaloicon from "../assets/zalo-icon.png"
import zl from '../assets/zalo.jpg'


const Intro = () => {
    const handleOpenModal = (id: string) => {
        if (document) {
            let a: any = document?.getElementById(id)
            a?.showModal()
        }

    }
    return (
        <div className='flex flex-col justify-center items-center gap-3  '>
            <div className="relative">

                <motion.img
                    initial={{ opacity: 0, y: 100 }}
                    animate={{ opacity: 1, y: 0 }}
                    className='h-40 aspect-square object-fill object-center  rounded-full shadow-2xl bg-gray-300 animate-pulsing  p-2'
                    src='./logo.jpg' />
                <label className="swap swap-rotate top-2 absolute -right-7 z-10">
                    {/* this hidden checkbox controls the state */}
                    <input type="checkbox" className="theme-controller" value="sunset" />

                    {/* sun icon */}
                    <svg
                        className="swap-off h-6 w-8 fill-current"
                        xmlns="http://www.w3.org/2000/svg"
                        viewBox="0 0 24 24">
                        <path
                            d="M5.64,17l-.71.71a1,1,0,0,0,0,1.41,1,1,0,0,0,1.41,0l.71-.71A1,1,0,0,0,5.64,17ZM5,12a1,1,0,0,0-1-1H3a1,1,0,0,0,0,2H4A1,1,0,0,0,5,12Zm7-7a1,1,0,0,0,1-1V3a1,1,0,0,0-2,0V4A1,1,0,0,0,12,5ZM5.64,7.05a1,1,0,0,0,.7.29,1,1,0,0,0,.71-.29,1,1,0,0,0,0-1.41l-.71-.71A1,1,0,0,0,4.93,6.34Zm12,.29a1,1,0,0,0,.7-.29l.71-.71a1,1,0,1,0-1.41-1.41L17,5.64a1,1,0,0,0,0,1.41A1,1,0,0,0,17.66,7.34ZM21,11H20a1,1,0,0,0,0,2h1a1,1,0,0,0,0-2Zm-9,8a1,1,0,0,0-1,1v1a1,1,0,0,0,2,0V20A1,1,0,0,0,12,19ZM18.36,17A1,1,0,0,0,17,18.36l.71.71a1,1,0,0,0,1.41,0,1,1,0,0,0,0-1.41ZM12,6.5A5.5,5.5,0,1,0,17.5,12,5.51,5.51,0,0,0,12,6.5Zm0,9A3.5,3.5,0,1,1,15.5,12,3.5,3.5,0,0,1,12,15.5Z" />
                    </svg>

                    {/* moon icon */}
                    <svg
                        className="swap-on h-6 w-8 fill-current"
                        xmlns="http://www.w3.org/2000/svg"
                        viewBox="0 0 24 24">
                        <path
                            d="M21.64,13a1,1,0,0,0-1.05-.14,8.05,8.05,0,0,1-3.37.73A8.15,8.15,0,0,1,9.08,5.49a8.59,8.59,0,0,1,.25-2A1,1,0,0,0,8,2.36,10.14,10.14,0,1,0,22,14.05,1,1,0,0,0,21.64,13Zm-9.5,6.69A8.14,8.14,0,0,1,7.08,5.22v.27A10.15,10.15,0,0,0,17.22,15.63a9.79,9.79,0,0,0,2.1-.22A8.11,8.11,0,0,1,12.14,19.73Z" />
                    </svg>
                </label>

            </div>


            <code className='break-before-avoid'>
                <p className='text-blue-400 text-xl animate-sway'>Hello các bạn</p> Cảm ơn vì đã đến đây
            </code>
            <div className='lg:text-3xl text-xl font-bold text-center'>
                Kết nối & Ủng hộ
            </div>
            <div className="flex gap-2 items-center">
                <a target="_blank" href="https://www.facebook.com/profile.php?id=100004445561665&locale=vi_VN" className="animate-blurred-fade-in">
                    <img src={fb} alt="" className="h-20" />
                </a>
                <div>

                    <img src={zaloicon} alt="" className="h-14 animate-tilt cursor-pointer" onClick={() => handleOpenModal('my_modal_3')} />

                    <dialog id="my_modal_3" className="modal">
                        <div className="modal-box">
                            <form method="dialog">
                                {/* if there is a button in form, it will close the modal */}
                                <button className="btn btn-sm btn-circle btn-ghost absolute right-2 top-2">✕</button>
                            </form>
                            <div className='flex flex-col'>
                                <h3 className="font-bold text-lg">Quét mã QR </h3>
                                <img className="py-4" src={zl} />
                            </div>
                        </div>

                    </dialog>
                </div>

            </div>
        </div >
    );
};

export default Intro;